...
const IndexedDBService = {
  ...
  save: async (key, data) => {
    ...
    return new Promise((resolve, reject) => {
      const transaction = IndexedDBService.db.transaction([IndexedDBService.STORE_NAME], 'readwrite');
  ...
  load: async (key) => {
    if (!IndexedDBService.db) await IndexedDBService.init();
    return new Promise((resolve, reject) => {
      const transaction = IndexedDBService.db.transaction([IndexedDBService.STORE_NAME], 'readonly');
      const store = transaction.objectStore(IndexedDBService.STORE_NAME);
...
const App = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const stored = await IndexedDBService.load('ft-products');
      if (stored) {
        setProducts(stored);
      } else {
        const defaults = [
        ...
        await IndexedDBService.save('ft-products', defaults);
      }
  ...
  const handleSave = async () => {
    ...
    await IndexedDBService.save('ft-products', newProducts);
